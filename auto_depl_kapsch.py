...
def create_network():
    ...
    # change this
    expiration = int(j.data.time.HRDateToEpoch('2020/09/12'))
...
def create_minio(nodeset):
    ...
    password = "kapsch12345"  # zdb secret_set
    ...
    minio_secret_encrypted = j.sal.zosv2.container.encrypt_secret(minio_master_node_id, "kapsch12345")
...
def create_container(interact):
    ...
    wallet = j.clients.stellar.get(wallet_name)
    expiration = int(j.data.time.HRDateToEpoch('2020/08/08'))
...
overlay_network_name="kapsch-sbg-net"
overlay_network_pre="10.22."  # / 16
